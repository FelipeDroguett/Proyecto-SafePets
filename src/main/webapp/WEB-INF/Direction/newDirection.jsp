...
<div class="container">
		...
		<div>
			<form:label path="foundation">fundacion</form:label>
			<form:select path="foundation" class="form-select">
				<c:forEach items="${foundations}" var="fou">
					<form:option value="${fou.id}">${fou.firstName}</form:option>
				</c:forEach>
			</form:select>
			<form:errors path="foundation" class="text-danger"/>
		</div>
